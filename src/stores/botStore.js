...
export const useBotStore = create((set, get) => ({
  ...
  // 3s cost of the full sessions endpoint (no usage/cost enrichment).
  ...
  // Full session fetch — includes cost/token enrichment. Used by Agent Monitor.
  fetchSessions: async () => {
    try {
  ...
  // Uses the lightweight status endpoint to avoid the ~3s full-sessions latency.
  startSessionPolling: () => {
    const state = get();
