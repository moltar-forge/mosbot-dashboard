  ...
  if (!Array.isArray(leadership)) {
    ...
    leadership.forEach((leader, idx) => {
      ...
      if (!leader.title || typeof leader.title !== 'string') {
        errors.push(`leadership[${idx}] (${leader.id || 'unknown'}): title is required and must be a string`);
      }
      
      if (!leader.label || typeof leader.label !== 'string') {
        errors.push(`leadership[${idx}] (${leader.id || 'unknown'}): label is required and must be a string`);
      }
      
      // Validate status if present
      if (leader.status && !['human', 'scaffolded', 'deprecated', 'active'].includes(leader.status)) {
        errors.push(`leadership[${idx}] (${leader.id || 'unknown'}): status must be one of: human, scaffolded, deprecated, active`);
      }
      
      // Validate reportsTo if present
      if (leader.reportsTo !== null && leader.reportsTo !== undefined) {
        if (typeof leader.reportsTo !== 'string') {
          errors.push(`leadership[${idx}] (${leader.id || 'unknown'}): reportsTo must be a string or null`);
          ...
          if (!referencedLeader) {
            errors.push(`leadership[${idx}] (${leader.id || 'unknown'}): reportsTo "${leader.reportsTo}" does not reference a valid leadership id`);
          }
        }
      }
    });
  ...
  if (!Array.isArray(departments)) {
    ...
    departments.forEach((dept, idx) => {
      ...
      if (!dept.name || typeof dept.name !== 'string') {
        errors.push(`departments[${idx}] (${dept.id || 'unknown'}): name is required and must be a string`);
      }
      
      if (!dept.leadId || typeof dept.leadId !== 'string') {
        errors.push(`departments[${idx}] (${dept.id || 'unknown'}): leadId is required and must be a string`);
      } else if (!leadershipIds.has(dept.leadId)) {
        errors.push(`departments[${idx}] (${dept.id || 'unknown'}): leadId "${dept.leadId}" does not reference a valid leadership id`);
      }
  ...
  if (!Array.isArray(subagents)) {
    ...
    subagents.forEach((agent, idx) => {
      ...
      if (!agent.displayName || typeof agent.displayName !== 'string') {
        errors.push(`subagents[${idx}] (${agent.id || 'unknown'}): displayName is required and must be a string`);
      }
      
      if (!agent.label || typeof agent.label !== 'string') {
        errors.push(`subagents[${idx}] (${agent.id || 'unknown'}): label is required and must be a string`);
      }
    });
      ...
      if (Array.isArray(dept.subagents)) {
        dept.subagents.forEach((subagentId, subIdx) => {
          if (!subagentIds.has(subagentId)) {
            errors.push(`departments[${deptIdx}] (${dept.id || 'unknown'}): subagents[${subIdx}] references unknown subagent id "${subagentId}"`);
          }
        });
      }
    });
